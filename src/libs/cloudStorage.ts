...
const bucket = cloudStorage.bucket(
  process.env.GCP_STORAGE_BUCKET_NAME as string
);
...
export const generateUploadSignedUrl = async (fileKey: string) => {
...
export const uploadToGCS = ({
  ...
  return new Promise((resolve, reject) => {
    ...
    const writeStream = ref.createWriteStream({
      metadata: { contentType: mimetype || undefined },
    });
    ...
    writeStream.on("finish", async () => {
      ...
      await ref.setMetadata({
        metadata: { originalFilename: originalFilename || "unknown" },
      });
      resolve(fileKey);
    });
  });
